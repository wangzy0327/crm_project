    ...
    $.ajax({
        ...
        success:function (result) {
            if(result.code == 0){
                ...
                $($('.note-editable')[0]).html(data.title);
                $($('.note-editable')[1]).html(data.description);
                var picUrl = data.picUrl;

                var $li = $('<div id="' + picUrl + '" class="file-item thumbnail">' +
                    '<img src="'+picUrl+'" style="width: 100px;height: 100px;">' +
                    '<div class = "file-panel" style = "height: 30px;text-align: center;" >  ' +
                    '<a class = "delfile" style="display:block;text-align: center;">删除</a>' +
                    '<span class="info" style="text-align: center;">' + picUrl + '</span></div>' +
                    '</div>'+
                    '</div>'),
                    $img = $li.find('img');
                $('#thelist').append($li);
...
function saveContent() {
    $('#saveRichText').click(function () {
        ...
        $.ajax({
            url: "/message/richText/update?tags="+tags,
            type: "POST",
            dataType: 'json',
